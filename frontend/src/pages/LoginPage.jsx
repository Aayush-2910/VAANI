...
function LoginPage() {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.role === 'admin') {
          ...
          // Redirect to admin-dashboard app running on port 3003
          ...
          sessionStorage.setItem('userName', result.user.displayName || formData.email.split('@')[0]);
          sessionStorage.setItem('userEmail', formData.email);
          // Redirect to volunteer-dashboard app running on port 3002
          ...
          sessionStorage.setItem('userName', result.user.displayName || formData.email.split('@')[0]);
          sessionStorage.setItem('userEmail', formData.email);
          // Redirect to user-dashboard app running on port 3001
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result.success) {
        if (result.needsRole) {
          // New user needs to select role - redirect to signup
          navigate('/signup')
        } else {
